...
df_follow = df_follow.rename(columns = { "follow_count":"total_follow" })

df_hype = df_main.groupby(["genre_name"])["hype_count"].sum().reset_index()

df_hype = df_hype.rename(columns = { "hype_count": "total_hypes" })

plt.bar(df_follow.index - BAR_WIDTH / 2, df_follow["total_follow"], color = "blue", labal = "Number of Follow", width = BAR_WIDTH)
...
df_main = pd.read_csv("game-ratings-by-top-10-platform.csv")
